...
## ssh keys
For any interaction with the pipeline server (except of mounting the SAMBA share), you have to authenticate. The pipeline server is setup to only accept passwordless logins, which authenticates by using a so called ssh certificate, which is unique to your computer. This is safer than password authentication and much easier to use once setup. 
...
After you have an ssh key for your computer, you have to contact me to have it added to the instance so that you can interact with the pipeline server either via the local pipeline management scripts or logging in using ssh (username `ubuntu`) and executing commands directly in the pipeline server.
